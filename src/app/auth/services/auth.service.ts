...
export class AuthService implements IAuthRepository {
  constructor(private router: Router, @Inject('localSRepository') private localStorageService: ILocalSRepository) { }
  ...
  validation(username: string, password: string): IUser | undefined {
    if (!localStorage.hasOwnProperty("arrayInfoUsers")) {
      ...
      return arrayInfoUsers.find(element => element.username === username && element.password === password);
    } else {
